...
plot(f_corr, 10*log10(S_corr), 'c', 'LineWidth', 1.2); hold on;
yline(10*log10(sigma^2), 'r--', 'DSP théorique');
xlabel('Fréquence (Hz)');
ylabel('DSP (dB)');
title('Corrélogramme');
...
%%  Affichage 
figure;
histogram(SF_vals, 15, 'FaceColor', 'b');
title('Périodogramme brut');
xlabel('Platitude spectrale'); ylabel('Occurrences');
xlim([0 1]); grid on;

figure;
histogram(SF_welch_vals, 15, 'FaceColor', 'g');
title('Méthode Welch (Mon\_Welch)');
xlabel('Platitude spectrale'); ylabel('Occurrences');
xlim([0 1]); grid on;

figure;
histogram(SF_p_vals, 15, 'FaceColor', 'm');
title(sprintf('Platitude généralisée (p = %.1f)', p));
xlabel('Platitude'); ylabel('Occurrences');
xlim([0 1]); grid on;
